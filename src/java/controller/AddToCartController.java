...
public class AddToCartController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Dvdcatalog catalog_id = DvdCatalogTable.findCatalogById(dvdId);
   
        List<Cart> old_cart = CartTable.findCartByIdCatalog(catalog_id);
            
        if (request.getParameter("additem") != null && old_cart == null) {
            cart.setQuantity(Integer.parseInt(request.getParameter("quantity")));
            cart.setDvdid(catalog_id);
            CartTable.insertCart(cart);
        }else if(request.getParameter("additem") != null && old_cart != null){
            cart.setCartid(old_cart.get(0).getCartid());
            cart.setDvdid(catalog_id);
